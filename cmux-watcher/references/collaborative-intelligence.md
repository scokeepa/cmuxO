...
> Boss(Orchestrator)과 Watcher(Sentinel) 간의 고급 협력 프로토콜 8가지.
...
### 문제
Boss가 매번 `read-screen --scrollback --lines 20`으로 DONE x2, 5 blank lines를 직접 확인해야 함.
...
eagle 감지: "DONE" 키워드 발견
  → Watcher 추가 검증:
    ...
    2. "DONE" 2회 존재 확인
    3. summary 텍스트 존재 확인
  → 검증 성공: [WATCHER→BOSS] DONE_VERIFIED: surface:7 완료, DONE x2 확인, summary 있음
  → 검증 실패: [WATCHER→BOSS] DONE_PARTIAL: surface:7 DONE 1회만 (프로토콜 미준수), scrollback 필요
```
...
| TYPE | 의미 |
|------|------|
| **DONE_VERIFIED** | DONE x2 + summary 확인 완료. Boss가 바로 merge-judge 진행 가능 |
| **DONE_PARTIAL** | DONE 키워드 있지만 프로토콜 미충족. Boss가 직접 확인 필요 |

### Boss 행동 변경
```
기존: DONE 감지 → Boss가 read-screen으로 직접 확인 → merge-judge
개선: DONE_VERIFIED 수신 → Boss가 바로 merge-judge (검증 스킵)
     DONE_PARTIAL 수신 → Boss가 read-screen으로 추가 확인
```
...
### 문제
Watcher가 RATE_LIMITED 감지 → Boss에 알림 (비동기). 알림 전달 전에 Boss가 해당 surface에 배정할 수 있음.

### 해결
`/tmp/cmux-rate-limited-pool.json` 공유 파일. Watcher가 즉시 기록, Boss가 배정 전 즉시 체크.
...
### 문제
Worker 에러 시 Watcher→Boss 보고→Boss 복구→Boss 재배정으로 4단계. 지연 발생.
...
| 에러 유형 | Watcher 직접 복구 | Boss 에스컬레이션 |
|----------|------------------|-----------------|
...
if echo "$variable_screen" | grep -qE "❯|Type your message"; then
    # [WATCHER→BOSS] AUTO_RECOVERED: surface:8 API 529 → /new 복구 성공. 재배정 대기.
else
    # [WATCHER→BOSS] RECOVERY_FAILED: surface:8 /new 후에도 비정상. Boss 개입 필요.
fi
```
...
| TYPE | 의미 |
|------|------|
| **AUTO_RECOVERED** | Watcher가 직접 복구 성공. Boss는 재배정만 하면 됨 |
| **RECOVERY_FAILED** | Watcher 복구 실패. Boss 에스컬레이션 필요 |
...
| TYPE | 의미 |
|------|------|
| **UNRELIABLE** | surface가 30분간 3회+ 에러. Boss가 배정 회피 권장 |
...
### Watcher IDLE 보고 강화
```
기존: [WATCHER→BOSS] IDLE: surface:3,10 유휴 90초+
개선: [WATCHER→BOSS] IDLE_WITH_QUEUE: surface:3,10 유휴 90초+, 2 tasks pending
  또는
      [WATCHER→BOSS] IDLE_NO_QUEUE: surface:3,10 유휴 90초+, 0 tasks pending (라운드 종료 가능)
```
...
### 해결
Boss가 라운드 단계를 Watcher에 알려주면, Watcher가 단계에 맞게 감시 강도 조절.
...
| **MERGING** | Boss 건강 체크만 | 120초 | Worker 비활성, Boss 집중 필요 |
| **IDLE** | 전체 상태 | 60초 | 라운드 사이 |

### 메시지 흐름
```
Boss: [BOSS→WATCHER] PHASE_CHANGE: DISPATCH (4 surfaces dispatched)
Watcher: 폴링 15초로 전환, QUEUED/STUCK 집중 감시

Boss: [BOSS→WATCHER] PHASE_CHANGE: WORKING
Watcher: 폴링 60초로 전환, STALLED/ERROR 집중

Boss: [BOSS→WATCHER] PHASE_CHANGE: COLLECTING
Watcher: 폴링 30초로 전환, DONE 감지 집중

Boss: [BOSS→WATCHER] PHASE_CHANGE: MERGING
Watcher: 폴링 120초로 전환, Boss 건강만 체크
```
...
| TYPE | 의미 |
|------|------|
| **PHASE_CHANGE** | Boss가 라운드 단계 변경 알림 |
...
### HEARTBEAT 확장
```
기존: [WATCHER→BOSS] HEARTBEAT: 12 surfaces | W:5 I:3 D:2 E:1 S:1
개선: [WATCHER→BOSS] HEARTBEAT: 12 surfaces | W:5 I:3 D:2 E:1 S:1 | best:surface:2(0.95) worst:surface:7(0.62)
```
...
| TYPE | 의미 |
|------|------|
| **SURFACE_PROFILE** | 특정 surface 성능 상세 (Boss 요청 시) |
...
### 문제
DONE 확인 후 Boss가 수동으로 `/clear` → sleep 5 → dispatch. 매번 30초+ 소요.

### 해결
Watcher가 DONE_VERIFIED 후 `/clear`까지 수행하고 "준비 완료" 보고. Boss는 바로 dispatch만.
...
if echo "$variable_screen" | grep -qE "❯|Type your message"; then
    # [WATCHER→BOSS] DONE_READY: surface:7 완료 검증 + /clear 완료. 즉시 dispatch 가능.
else
    # [WATCHER→BOSS] DONE_VERIFIED: surface:7 완료 검증. /clear 실패 — Boss 직접 처리.
fi
```
...
| TYPE | 의미 |
|------|------|
| **DONE_READY** | DONE 검증 + /clear 완료. Boss는 바로 dispatch만 하면 됨 |
...
- DONE_READY 전송 후 Boss가 배정하지 않으면 → 90초 후 IDLE로 재보고
...
| RECOVERY_FAILED | 3 | Watcher 복구 실패, Boss 개입 필요 |
...
### Watcher → Boss (기존, 유지)
DONE, IDLE, ERROR, STALLED, RATE_LIMITED, WAITING, RECOVERED, HEARTBEAT, BOSS_DOWN
...
| `/tmp/cmux-rate-limited-pool.json` | Watcher | Boss (읽기) | Rate limit 즉시 동기화 |
| `/tmp/cmux-error-history.jsonl` | Watcher | Watcher (분석) | 에러 빈도 추적 |
| `/tmp/cmux-surface-profiles.json` | Watcher | Boss (읽기) | Surface 성능 프로파일 |
