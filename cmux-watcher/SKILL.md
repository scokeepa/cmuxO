...
> CCTV가 도둑을 직접 잡지 않듯, 감사실은 Main(COO)에게 보고만 한다.
...
- **MEMBER REQUEST 감지**: 팀장의 팀원 요청 패턴 감지 → Main에 전달
...
│
├─ 2. NOTIFY: Main에 cmux send + enter (DONE 1개 확정될 때마다 즉시 개별 보고)
│
├─ 3. READ: Main 화면 읽기 (3초 대기 후 read-surface)
│     → Main WORKING? → 감시 계속 (60초 간격)
│     → Main IDLE (사용자 질문 중)? → 느린 폴링 (120초)
│     → Main IDLE + workers IDLE? → 대기 모드 (120초)
│     → Main WORKING + workers IDLE? → 빠른 감시 (15초, 배정 시작 감지)
│
...
- **팀원 surface에 cmux send로 지시/명령 전송** ← **가장 심각한 위반! Watcher는 관찰+보고만. 고장 시에도 Main에 보고하지 직접 개입 금지**
- **팀원에게 `/new`, `/clear`, 작업 내용 전송** ← 고장 복구도 Main이 판단
- SCAN만 하고 NOTIFY 생략
- NOTIFY만 하고 READ 생략 ← Main 반응 무시하고 넘어가기
...
> **Watcher = CCTV.** CCTV가 도둑을 직접 잡지 않는다. 경비실(Main)에 알릴 뿐이다.
> 고장 감지 → Main에 보고. Main이 복구 지시. Watcher가 직접 복구 시도 절대 금지.

## ⚡ IDLE 재배정 촉구 (GATE W-10 — 강제 + Debounce)

> **놀고 있는 surface 발견 시 Main에 "다음 작업 배정하세요!" 반드시 포함.**
...
> **교훈**: Main이 dispatch 직후에도 와쳐가 IDLE 재촉을 보내서 노이즈 발생.
> Dispatch 후 30초 grace period 필요.

**규칙:**
1. **DONE 보고 후 30초 grace** — DONE 보고 직후 30초간 해당 surface의 IDLE 재촉 금지 (Main이 재배정 중)
...
- "Main이 알아서 하겠지" 합리화
- **DONE 1회 보고 후 재촉 안 함** ← Main이 배정 안 하면 매 라운드마다 재촉 필수
...
- 상태 변화 알림 생성 (cmux send + enter → Main)
- **알림 후 Main 반응 읽기** (read-surface → 상태 파악)
- **Main 상태에 따라 폴링 간격 자동 조절** (adaptive polling)
- Main 장애 감지 + 복구 시도
- cmux 사이드바 상태 시각화
- **스캔 완료 후 Main surface에 /cmux 엔터 전송** (detect-surface-models.py 자동)

## Adaptive Polling (v4.0 신규 — 강제)

| Main 상태 | 팀원 상태 | 폴링 간격 | 이유 |
|-----------|------------|----------|------|
| WORKING | WORKING | 60초 | 정상 감시 |
| IDLE | WORKING | 30초 | 곧 완료될 수 있음 |
| IDLE | ALL IDLE | 120초 | 모두 대기 — 사이드바 회색 |
| WORKING | ALL IDLE | 15초 | Main 배정 시작 — 빠른 감지 |

> `/tmp/cmux-watcher-state.json`에 현재 상태 기록 (main_state, interval, timestamp)
...
**감지 결과:** 모델(Opus/Sonnet/Codex/GLM/MiniMax 등) + 상태(IDLE/WORKING/ERROR/DONE) + 역할(main/watcher/worker, `/tmp/cmux-roles.json` 참조)
...
| 역할 | 전문 영역 | 상대에 대한 인식 |
|------|----------|----------------|
| **Orchestrator (Main)** | 계획, 배정, 병합, 커밋 | Watcher = 감시 동료, 알림 신뢰 |
...
### Inter-Peer Communication (Watcher ↔ Main 소통 규약)
...
```
[WATCHER→MAIN] DONE: surface:7 완료         ← Watcher가 Main에게
[WATCHER→MAIN] IDLE: surface:3,10 유휴 90초+ ← 작업 배정 필요
[WATCHER→MAIN] ERROR: surface:8 API 529      ← 즉시 복구 필요
[MAIN→WATCHER] ACK: surface:7 확인           ← Main 응답
사용자: surface:3 상태 알려줘                  ← 접두사 없음 = 사용자
```

**Watcher가 보내는 유형:**
- 기본: DONE, IDLE, ERROR, STALLED, RATE_LIMITED, WAITING, RECOVERED, HEARTBEAT, MAIN_DOWN
...
| 우선순위 | TYPE | 전달 |
|---------|------|------|
| CRITICAL | ERROR, RATE_LIMITED, MAIN_DOWN | SendMessage + cmux notify + cmux log |
...
Watcher는 Main surface도 감시 대상에 포함한다.

```bash
# Main 생존 확인 (2분마다)
RESULT=$(bash "$ROLE_SCRIPT" check-main)
case "$RESULT" in
  MAIN_ALIVE*)   ;; # 정상
  MAIN_DEAD*)
    ...
    # 2단계: 복구 시도
    cmux send --workspace "$MAIN_WS" --surface "$MAIN_SF" "/compact"
    cmux send-key --workspace "$MAIN_WS" --surface "$MAIN_SF" enter
    sleep 5

    # 3단계: 작업 재개 지시
    cmux send --workspace "$MAIN_WS" --surface "$MAIN_SF" "이전 작업을 이어서 진행해. role-register.sh heartbeat main 실행 후 계속."
    cmux send-key --workspace "$MAIN_WS" --surface "$MAIN_SF" enter
    ;;
  MAIN_NOT_REGISTERED*)
    # Main 미등록 → 알림만
    cmux notify --title "WATCHER" --body "Main이 등록되지 않음. cmux-orchestrator 시작 필요."
    ;;
esac
```
...
| 역할 | 자기 인식 | 상대 인식 | 행동 |
|------|----------|----------|------|
| **Main** | "나는 오케스트레이터" | Watcher에 작업 배정 금지 (GATE 15) | worker 목록에서 watcher 제외 |
| **Watcher** | "나는 감시자" | Main surface 추가 감시 | Main DEAD 시 복구 시도 |
| **팀원** | 역할 없음 | Main/Watcher 모름 | 받은 작업만 수행 |
...
```bash
# 60초 주기 연속 스캔 (JSON + Main 알림)
python3 ~/.claude/skills/cmux-watcher/scripts/watcher-scan.py --continuous 60 --notify-main --json
...
| # | 기능 | Watcher 행동 | Main 이점 |
|---|------|-------------|----------|
| 1 | **DONE x2 검증** | DONE 감지 → scrollback으로 x2 확인 → DONE_VERIFIED/DONE_PARTIAL 보고 | Main read-screen 생략 |
| 2 | **Rate Limit Pool** | 감지 즉시 `/tmp/cmux-rate-limited-pool.json` 기록 | Main 배정 전 체크 가능 (경쟁조건 제거) |
| 3 | **~~팀원 자동 복구~~** | ⛔ **폐지 (GATE W-9)** — 고장 감지 시 Main에 보고만. 직접 `/new` 전송 금지. | Main이 복구 판단 |
| 4 | **에러 빈도 추적** | `/tmp/cmux-error-history.jsonl` 축적 → 30분 3회+ → UNRELIABLE 보고 | Main 불안정 surface 회피 |
| 5 | **태스크 큐 인식** | speckit-tracker 읽어 IDLE_WITH_QUEUE / IDLE_NO_QUEUE 구분 보고 | Main 라운드 종료 판단 지원 |
| 6 | **라운드 단계별 감시** | PHASE_CHANGE 수신 → 폴링 주기+초점 자동 조절 | 효율적 리소스 사용 |
| 7 | **Surface 프로파일** | 완료 시간, 에러율 축적 → HEARTBEAT에 best/worst 포함 | Main 배정 최적화 |
| 8 | **~~DONE 준비~~** | ⛔ **폐지 (GATE W-9)** — `/clear` 직접 전송 금지. Main에 DONE_VERIFIED 보고만. | Main이 clear 판단 |

### ⛔ 팀원 개입 절대 금지 (GATE W-9 — v4.1 신규)

> **Watcher = CCTV. 직접 개입 금지. 모든 액션은 Main에 보고 후 Main이 실행.**

| 상황 | Watcher 행동 | ❌ 금지 행동 |
|------|-------------|------------|
| 팀원 크래시 | Main에 ERROR 보고 | `/new` 직접 전송 |
| 팀원 멈춤 | Main에 STALLED 보고 | escape/interrupt 전송 |
| 팀원 완료 | Main에 DONE 보고 | `/clear` 전송 |
| 팀원 질문 대기 | Main에 WAITING 보고 | 답변 직접 전송 |
| Rate limit | Main에 RATE_LIMITED 보고 | surface 스킵 지시 |

### 라운드 단계별 폴링 주기

| Main이 보내는 PHASE | Watcher 폴링 | 감시 초점 |
|--------------------|------------|----------|
| DISPATCH | 15초 | QUEUED, STUCK |
| WORKING | 60초 | STALLED, ERROR |
| COLLECTING | 30초 | DONE 집중 |
| MERGING | 120초 | Main 건강만 |

---

## 감지 상태 및 알림 (→ Main 즉시 보고)

> 감지는 cmux-orchestrator의 eagle_watcher.sh + eagle_analyzer.py + vision-monitor.sh가 담당.
> watcher는 이들의 JSON 결과를 읽어 **알림 생성 + Main 보고**에 집중.

| 상태 | 우선순위 | Main 보고 | 의미 | 권장 액션 |
|------|---------|----------|------|-----------|
| **MAIN_DEAD** | CRITICAL | 즉시 | Main 하트비트 2분+ 없음 | `/compact` → 작업 재개 강제 |
...
| **STUCK_PROMPT** | HIGH | 즉시 | `›` 프롬프트에 셸 명령이 입력된 채 대기 | Main에 보고 → Esc×3 + /new 복구 |
| **WAITING** | HIGH | 즉시 | 사용자 입력 대기 (y/n 등) | 질문 확인 후 응답 전송 |
| **DONE** | HIGH | **즉시** | 작업 완료 → Main이 바로 다음 작업 배정 | 결과 리뷰 + 즉시 재배정 |
...
4. 해당 surface의 미완료 작업을 보고에 포함 → Main이 다른 surface에 재배정

### 알림 쿨다운 (중복 방지)

| 상태 | 쿨다운 | 이유 |
|------|--------|------|
| MAIN_DEAD | 2분 | 복구 시도 후 재판단 필요 |
| ERROR | 2분 | 복구에 시간 소요 |
...
**보고 형식:**
```
[WATCHER→MAIN] STUCK_PROMPT: s:N 프롬프트에 명령 잔류 (cd /path && npx...). Esc×3 + /new 필요.
```
...
| 결과 | 판정 | 신뢰도 | 행동 |
|------|------|--------|------|
| 텍스트 동일 (시간/숫자 제외) | **STALLED** | 0.95 | 정밀 조사 + Main 알림 |
...
```bash
# 메인 surface(role-register에서 main으로 등록된 surface)의 화면 확인
MAIN_SF=$(python3 -c "import json; r=json.load(open('/tmp/cmux-roles.json')); print(r.get('main',{}).get('surface',''))" 2>/dev/null)

if [ -n "$MAIN_SF" ]; then
    MAIN_SCREEN=$(bash "$ORCH_DIR/scripts/read-surface.sh" "${MAIN_SF#surface:}" --lines 5)
    # "/cmux" 입력 대기 중이면 엔터 쳐줌
    if echo "$MAIN_SCREEN" | grep -q "/cmux"; then
        MAIN_WS=$(python3 -c "import json; r=json.load(open('/tmp/cmux-roles.json')); print(r.get('main',{}).get('workspace',''))" 2>/dev/null)
        cmux send-key --workspace "$MAIN_WS" --surface "$MAIN_SF" enter
        sleep 3
        # 메인에게 스캔 결과 알림
        cmux notify --title "Watcher Ready" --body "스캔 완료. /tmp/cmux-surface-scan.json 참조." --workspace "$MAIN_WS"
    fi
fi
```
...
| W-6 | **Main Never Blocked** | watcher는 항상 백그라운드, 메인 작업 차단 금지 |
...
```bash
cmux send --workspace WS --surface SF "/compact"                  # Main 복구
