...
## Boss(사장) 역할 제한 (MANDATORY — Iron Rule)

Boss는 **COO(최고운영책임자)**이다. 직접 실무를 하지 않는다.

### Boss가 해야 할 것 (ONLY)
1. **작업 분석 → 부서 편성** (어떤 부서가 필요한지 판단)
2. **부서에 지시** (cmux send로 작업 전달)
3. **결과 취합** (DONE 수집 + 종합 판단)
4. **최종 커밋** (git commit은 Boss만 수행)

### Boss가 하면 안 되는 것 (NEVER)
- 직접 코드 읽기/분석
- 직접 리서치/조사
- 직접 갭 분석/품질 검증
- 직접 구현/디버깅
...
> 부서가 분석 결과를 DONE으로 보고하면 Boss가 취합하여 다음 판단을 내린다.
> Boss의 컨텍스트는 분배와 취합에만 사용한다.
...
Boss의 잘못된 행동:
  ✗ Boss가 직접 코드를 읽어서 갭 분석
  ✗ Boss가 직접 리서치 수행
  ✗ Boss가 분석 끝난 후에야 부서 편성
```
...
**DONE 보고를 그대로 믿지 않는다.** Boss는 부서 DONE 수집 시 반드시 실제 결과물을 검증한다.
...
### Boss가 팀장에게 지시할 때 포함할 문구 (MANDATORY)

Boss는 부서에 작업 전달 시 반드시 다음 문구를 포함한다:
...
팀장은 작업 분석 후 자체 판단으로 팀원을 생성한다. Boss에 요청하지 않고 직접 pane을 분할한다.
...
Boss가 dispatch 시 **스킬 컨텍스트를 자연어로 프롬프트에 포함**하여 전송한다.
...
> Boss의 SKILL.md에 있는 규칙 중 Worker에게 공유해야 할 것만 추출하여 전달.
...
| 권한 거부 | `Operation not permitted`, `sandbox` | Boss 직접 처리 |
